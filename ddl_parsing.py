...
class DdlParse(DdlParseBase):
    """DDL parser"""
    _drop=Suppress(OneOrMore(Word(alphas))+("`")+Word(alphas)+("`")+(";"))
    ...
    _CREATE_TABLE_STATEMENT = Suppress(_CREATE) + Optional(_TEMP)("temp") + Suppress(_TABLE) +Suppress("public.") + Optional(Suppress(CaselessKeyword("IF NOT EXISTS"))) \
                ...
                Group(
                    ...
                    + (
                        ...
                        (
                            (_FOREIGN_KEY)("type")
                            + _LPAR + Group(delimitedList(Optional(_SUPPRESS_QUOTE) + Word(alphanums + "_") + Optional(_SUPPRESS_QUOTE)))("constraint_columns") + _RPAR
                            + Optional((_REFERENCES)
                                +Suppress("public.") + Optional(_SUPPRESS_QUOTE) + Word(alphanums + "_")("references_table") + Optional(_SUPPRESS_QUOTE)
    ...
    _DDL_PARSE_EXPR << OneOrMore(_CREATE_TABLE_STATEMENT)


    def __init__(self, ddl=None, source_database=None,fk_data=None):
        super().__init__(source_database)
        self._ddl = ddl
            ...
            if ret_col.getName() == "column":
                ...
                for col_name in ret_col["constraint_columns"]:
                    ...
                    if ret_col["type"] == "FOREIGN KEY":
                        # fk_info = ["tableName", "columnName"]
                        # tableName = ret['constraint']["references_table"]
                        # columnName = ret['constraint']["references_columns"][-1]
                        # fk_ref = dict(zip(fk_info, (tableName, columnName)))
                        col.not_null = True
                        col.foreign_key = self._fk_data[count]
                        count+=1
